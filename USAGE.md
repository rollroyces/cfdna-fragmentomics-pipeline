...
- **`cfdna-fragmentomics-pipeline`** (this repo) — data download,
  feature extraction, and the headline tumor-naive fragmentomics
  classifier. **This is where most users will start.**
- **`DeepCatch`** — the panel-LLR mutation-informed detector, plus
  cross-repo fusion of mutation + tumor-naive channels.
...
the **model card** is in DeepCatch's [`MODEL.md`](../DeepCatch/MODEL.md).
...
### 1.4 Install DeepCatch (only if doing fusion / panel LLR work)
...
`deepcatch-fusion`, `deepcatch-decisioncurve`.
